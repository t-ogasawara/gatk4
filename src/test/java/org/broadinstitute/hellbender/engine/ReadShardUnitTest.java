...
public class ReadShardUnitTest extends BaseTest {
    ...
    public void testShardInterval( final SimpleInterval originalInterval, final int windowSize, final int windowPadding, final ReadsDataSource readsSource, final SAMSequenceDictionary dictionary, List<ReadShard> expectedWindows  ) {
        final List<ReadShard> windows = ReadShard.shardInterval(originalInterval, windowSize, windowPadding, readsSource, dictionary);
        ...
        for ( int i = 0; i < windows.size(); ++i ) {
            final ReadShard window = windows.get(i);
    ...
    public void testShardIntervalThrowsOnInvalidContig() {
        ...
        ReadShard.shardInterval(new SimpleInterval("2", 1, 10), 100, 0, readsSource, dictionary);
    }
    ...
    public void testShardIntervalThrowsOnOutOfBoundsInterval() {
        ...
        ReadShard.shardInterval(new SimpleInterval("2", 1, 17000), 100, 0, readsSource, dictionary);
    }
}
