...
public final class HaplotypeCallerEngine implements AssemblyRegionEvaluator {
    ...
    private Set<GATKRead> filterNonPassingReads( final AssemblyRegion activeRegion ) {
        ...
        for( final GATKRead rec : activeRegion.getReads() ) {
            if( rec.getLength() < READ_LENGTH_FILTER_THRESHOLD || rec.getMappingQuality() < READ_QUALITY_FILTER_THRESHOLD || ! ReadFilterLibrary.MATE_ON_SAME_CONTIG.test(rec) || (hcArgs.keepRG != null && !rec.getReadGroup().equals(hcArgs.keepRG)) ) {
